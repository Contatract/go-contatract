...
type Backend interface {
	...
	GenerateShardingTxBatches(block *types.Block, stateDB *state.StateDB) (types.ShardingTxBatches, map[uint16]uint64, error)
}
...
func New(elephant Backend, config *params.ElephantChainConfig, mux *event.TypeMux, engine consensus.EngineCtt, eth *eth.Ethereum) *Miner {
	miner := &Miner{
		elephant: elephant,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, elephant, mux, eth),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(elephant.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	...
	log.Info("Starting sealing operation")
	self.worker.start()
...
func (self *Miner) IsStopped() bool {
	return self.stopped
}
...
func (self *Miner) GetStateDB() *state.StateDB {
	return self.worker.current.state
}
