...
func (stage *HBFTStageCompleted) String() string {
	return fmt.Sprintf("[viewID: %d, seqID: %d, msgType: %d\nhash: %s, parentStage: %s]",
		stage.ViewID, stage.SequenceID, stage.MsgType, stage.Hash().String(), stage.ParentStage.String())
}
...
type Header struct {
	...
	ParentNoBftHash common.Hash `json:"ParentNoBftHash"  gencodec:"required"`
...
func (h *Header) HashNoBft() common.Hash {
	return rlpHash([]interface{}{
		h.ParentHash,
		h.ParentNoBftHash,
		h.Coinbase,
...
func (b *Block) ParentNoBftHash() common.Hash { return b.header.ParentNoBftHash }
...
`, b.Number(), b.Size(), b.header.HashNoBft(), b.header, b.transactions, b.inputTxs, b.uncles, b.hbftStages)
	return str
}
...
[
	...
	BftHash:       %x
	MixDigest:      %x
]`, h.Hash(), h.ParentHash, h.ParentNoBftHash, h.Coinbase, h.Root, h.TxHash, h.ReceiptHash, h.Difficulty, h.Number, h.GasLimit, h.GasUsed, h.Time, h.Extra, h.LampBaseHash, h.LampBaseNumber, h.BftHash, h.MixDigest)
}
