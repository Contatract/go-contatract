...
func NewTransaction(nonce uint64, to common.Address, amount *big.Int, gasLimit uint64, gasPrice *big.Int, data []byte,
	actType uint8, actData []byte) *Transaction {
...
// Hash hashes the RLP encoding of election.
// It uniquely identifies the election.
func (elect *AddressElection) Hash() common.Hash {
	if hash := elect.hash.Load(); hash != nil {
		return hash.(common.Hash)
	}
	v := rlpHash(elect)
	elect.hash.Store(v)
