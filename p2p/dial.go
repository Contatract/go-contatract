...
type Device interface {
	...
	SetupConn(fd net.Conn, flags connFlag, dialDest *discover.Node, cbData interface{}, peerProtocol string, peerNotiChan chan interface{}) error
}
...
func newDialState(static []*discover.Node, bootnodes []*discover.Node, ntab discoverTable, maxdyn int,
	...
	for _, n := range static {
		s.addStatic(&discover.NodeCB{Node: n, Cb: nil, PeerProtocol: "", PeerChan: nil})
	}
	return s
}
...
func (s *dialstate) newTasks(nRunning int, peers map[discover.NodeID]*Peer, now time.Time) []task {
	...
	addDial := func(flag connFlag, n *discover.Node) bool {
		...
		s.dialing[n.ID] = flag
		newtasks = append(newtasks, &dialTask{flags: flag, dest: &discover.NodeCB{Node: n, Cb: nil, PeerProtocol: "", PeerChan: nil}})
		return true
	}
...
func (t *dialTask) Do(dev *Device) {
	...
	err := t.dial(dev, t.dest.Node, t.dest.Cb, t.dest.PeerProtocol, t.dest.PeerChan)
	if err != nil {
		...
		if _, ok := err.(*dialError); ok && t.flags&staticDialedConn != 0 {
			if t.resolve(dev) {
				...
				t.dial(dev, t.dest.Node, t.dest.Cb, t.dest.PeerProtocol, t.dest.PeerChan)
			}
		}
	}
}
...
func (t *dialTask) dial(d *Device, dest *discover.Node, cbData interface{}, peerProtocol string, peerNotiChan chan interface{}) error {
	switch dev := (*d).(type) {
	case *Server:
		...
		return dev.SetupConn(mfd, t.flags, dest, cbData, peerProtocol, peerNotiChan)
